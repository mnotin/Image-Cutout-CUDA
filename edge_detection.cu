...
void gaussian_blur(unsigned char *h_matrix, int matrix_width, int matrix_height) {
  ...
  float gaussian_blur_kernel[KERNEL_SIZE*KERNEL_SIZE] = {1/16.0, 2/16.0, 1/16.0, 
  ...
  cudaMemcpy(d_kernel, gaussian_blur_kernel, KERNEL_SIZE*KERNEL_SIZE * sizeof(int), cudaMemcpyHostToDevice);
  ...
  convolution<<<blocks, threads>>>(d_matrix, matrix_width, matrix_height, d_kernel, 3);
