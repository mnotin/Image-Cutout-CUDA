...
void ProcessingUnitDevice::cutout(unsigned char *h_rgb_image, unsigned char *h_edge_matrix, Dim matrix_dim, Vec2 start_pixel, int threshold) {
  ...
  int h_done = 0;
  Dim macro_matrix_dim;
  macro_matrix_dim.width = blocks.x;
  macro_matrix_dim.height = blocks.y;
  ...
  cudaMalloc(&d_micro_cutout_matrix, matrix_dim.width * matrix_dim.height * sizeof(char));
...
__global__ void draw_edges_on_cutout_matrix_kernel(unsigned char *edge_matrix, char *micro_cutout_matrix, Dim matrix_dim, Vec2 start_pixel, int threshold, char *macro_cutout_matrix) {
  ...
  micro_cutout_matrix[global_index.y*matrix_dim.width + global_index.x] = result;
...
__global__ void apply_macro_to_micro_cutout_matrix_kernel(char *macro_cutout_matrix, char *micro_cutout_matrix, Dim macro_matrix_dim, Dim micro_matrix_dim) {
  ...
  if (local_index.x == 0 && local_index.y == 0) {
    macro_matrix_content = macro_cutout_matrix[blockIdx.y*macro_matrix_dim.width + blockIdx.x];
  }

  __syncthreads();

  if (macro_matrix_content == 'M') {
    micro_cutout_matrix[global_index.y*micro_matrix_dim.width + global_index.x] = 'M';
  }
}
