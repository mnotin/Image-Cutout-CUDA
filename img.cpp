...
GrayImage* createPGM(int width, int height) {
    ...
    cudaMallocHost(&(img->data), img->width * img->height * sizeof(unsigned char));
...
RGBImage* createPPM(int width, int height) {
    ...
    cudaMallocHost(&(img->data), img->width * img->height * 3 * sizeof(unsigned char));
