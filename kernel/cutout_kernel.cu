...
__global__ void apply_cutout_kernel(char *cutout_matrix, unsigned char *output_image, dim3 image_dim, int2 start_pixel, bool targeting) { 
  int2 global_index = make_int2(threadIdx.x + (blockIdx.x * blockDim.x), threadIdx.y + (blockIdx.y * blockDim.y));
 
  if (global_index.x < image_dim.x && global_index.y < image_dim.y) {
    apply_cutout_core(global_index, cutout_matrix, output_image, image_dim, start_pixel, targeting);
  }
}
